...
function openLightbox(src, alt){
  ...
  img.onerror = () => {
    const fb = fallbackImageForSrc(src);
    if(img.src && img.src.endsWith(fb)) return;
    img.src = fb;
  };
  img.src = src;
  img.alt = alt || "";
...
function fallbackImageForSrc(src){
  // Prefer SVG placeholders
  if(src && src.includes('/placeholders/')) return 'assets/img/placeholders/heritage.svg';
...
function renderCarousel(container, images, altBase){
  ...
  main.onerror = () => {
    const fb = fallbackImageForSrc(main.src);
    if(main.src && main.src.endsWith(fb)) return;
    main.src = fb;
  };
  ...
  imgs.forEach((src,i)=>{
    ...
    ti.onerror = () => {
      const fb = fallbackImageForSrc(src);
      if(ti.src && ti.src.endsWith(fb)) return;
      ti.src = fb;
    };
